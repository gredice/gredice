...
`7a9154561`. All 14 budget and acceptance rows passed. This establishes
...
| Headless compile baseline | 3.663 ms median across three warmed 1,000-compile Node runs (3.451-3.714 ms) on local Apple M4 Pro/24 GiB. The exact production-Chromium matrix measured complete miss-resolution max / prepare-plus-lookup max/current / navigation-compile max at 8.2 / 4.8/4.8 / 0.5 ms for the worst case and 2.4 / 1.1/1.1 / 0.2 ms for the furnished house; all passed 100 ms gates. These are host/browser measurements, not constrained-mobile CPU results. |
| Avatar collision step | The exact matrix retained the 2 ms automated p95 gate. The furnished 100-cell solid-wall workload recorded 514 complete movement resolutions (31 during the held-key leg), 0.15 ms p95, 0.2 ms max, 304 collision primitives, and 220 buckets. Representative house two-view movement recorded 509 resolutions (66 during the held-key legs), 0.15 ms p95, 0.4 ms max, and moved 0.47 m in third-person plus 0.76 m in first-person; that timed row has no portal/interior witness and therefore makes no doorway-crossing claim. Four-rotation semantic checks and owned/public WebGL traversal remain supporting doorway correctness evidence rather than unprofiled timing claims. |
...
| Constrained-mobile browser budget | Twelve rows ran at 390x844, browser DPR 3 capped to effective DPR 1, auto-constrained tier, 1024 px shadows, and 5 s warmup/sample. Every row passed. Browser-rAF p95 ranged from 17.2 to 27.2 ms and every row recorded zero long tasks. The furnished worst-case navigation row recorded 18.2/27.3 ms p95/max; the furnished house normal row recorded 26.6/27.7 ms. These pass the 33.3 ms automated mobile p95 gate but are not physical-device results. |
| Matched desktop blank-shell overhead | At identical 1280x720, medium quality, ANGLE Metal, 30 FPS ambient scheduling, and zero active leases, the no-structure control versus blank-shell candidate measured browser-rAF p95 25.8/26.5 ms, rendered FPS 25.1/25.2, 100/100 draws per rendered frame, 5,016/5,016 triangles per rendered frame, and 2.50/2.51 ms GPU p95. All paired regression gates passed. The 33.3 ms absolute ceiling is only a headless responsiveness guard; the separate 16.7 ms physical desktop target remains unverified. |
| Interior/editor budget | The normal worst-case exterior submitted 34 visible props and suppressed 66; furnished cutaway submitted all 100. Worst-case edit churn recorded 63 editor actions at 17.5/43.5 ms p95/max and 3.2 ms final-Canvas pointer-resolution max, below the 100/500/100 ms gates. The lifecycle row proved its intended transient scheduler peak at 30/60/30 FPS and 0/1/0 leases; ambient rows remained 30/30/30 FPS with 0/0/0 leases for the entire sample. |
