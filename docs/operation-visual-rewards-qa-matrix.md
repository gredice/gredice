...
| Insect protection mesh | `insectMesh` | Exposed bed or field. | The same low-poly cover used for agrotextile appears and hides plants/weeds under covered positions. | It renders through the shared cover model, but remains in the independent insect-mesh reward family so agrotextile removal cannot clear it. | Persistent until a newer matching remove-insect-mesh reward. | `raisedBedAgrotextileRewards.unit.ts`; `RaisedBedFields.tsx`. |
| Remove insect protection mesh | `removeInsectMesh` | Insect-protection cover exists. | The shared-looking cover disappears unless an independently active agrotextile reward still covers the same position. | Removal clears only the matching insect-mesh family and scope; it must not clear agrotextile. | Persistent until a newer matching insect-mesh reward. | `raisedBedAgrotextileRewards.unit.ts`. |
