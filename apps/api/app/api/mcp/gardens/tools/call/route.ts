...
async function getOwnedGardenOrThrow(auth: MCPAuth, gardenId: string) {
    ...
    if (
        !GardenIdPattern.test(gardenId) ||
        !Number.isSafeInteger(parsedGardenId)
    ) {
    ...
    try {
        ...
        switch (name) {
            ...
            case 'gardens/list-raised-beds': {
                ...
                const garden = await getOwnedGardenOrThrow(
                    auth,
                    input.gardenId,
                );
                result = {
        ...
        const errorCode = isInvalidParams
            ? -32602
            : (toolError?.code ?? -32603);
