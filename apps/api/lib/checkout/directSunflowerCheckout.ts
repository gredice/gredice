...
export type DirectSunflowerCheckoutBatch = {
    atomicallyFulfilledItemIds: ReadonlySet<number>;
    pendingItems: readonly ShoppingCartItemWithShopData[];
    resolvedAmountsByCartItemId: ReadonlyMap<number, number>;
};

export type DirectSunflowerCheckoutAtomicBatch = {
    pendingCheckoutItems: readonly ShoppingCartItemWithShopData[];
    resolvedAmountsByCartItemId: ReadonlyMap<number, number>;
 ...
 * transaction normally stays limited to snapshot validation and the durable
 * sunflower debit. Callers may atomically fulfill database-only items before
 * commit; remaining fulfillment runs afterward while the process lock is held.
 */
export async function withDirectSunflowerCheckoutBatch<T>({
    ...
    atomicOperation?: (
        batch: DirectSunflowerCheckoutAtomicBatch,
        ...
        async () => {
            const prepared = await dependencies.withCheckoutCartItemLocks(
                ...
                async (db) => {
                            ...
                            await dependencies.spendSunflowersBatch(
                                ...
                                db,
                                {
                                    ...
                                    legacyCartSpend: {
                                        ...
                                        coveredItems: sunflowerItems.map(
                                            (item) => {
                                                const paymentState =
                    ...
                    const pendingCheckoutItems = allCheckoutItems.filter(
                        (item) =>
                            paymentStatesByCartItemId.get(item.id) ===
                            'pending',
                    );
                    const atomicallyFulfilledItemIds =
            ...
            return {
                ...
                value: await operation({
                    atomicallyFulfilledItemIds:
                        prepared.atomicallyFulfilledItemIds,
