...
import { LeafRakeFixture } from '../../../packages/game/tests/LeafRakeFixture';
...
async function checkRake(
    ...
    for (const item of data.clusters) {
        ...
        expect(item.triangles).toBe(590);
        expect(item.rotation).toBeCloseTo((rotation * Math.PI) / 2, 5);
        expect(item.minY).toBeCloseTo(raised ? 1.07 : 0.4, 3);
        expect(item.height).toBeGreaterThan(1.05);
        expect(item.height).toBeLessThanOrEqual(1.1);
...
for (const light of ['day', 'night'] satisfies ('day' | 'night')[]) {
    for (const rotation of [0, 1, 2, 3]) {
        test(`leaf rake ${light} ${rotation}`, async ({ mount, page }) => {
            const errors: string[] = [];
            page.on('pageerror', (error) => errors.push(error.message));
            const fixture = await mount(
                <LeafRakeFixture rotation={rotation} light={light} />,
            );
            await checkRake(fixture, page, rotation);
...
for (const light of ['cloudy', 'dusk', 'rain', 'snow'] satisfies (
    ...
    test(`leaf rake ${light}`, async ({ mount, page }) => {
        const fixture = await mount(
            <LeafRakeFixture rotation={0} light={light} />,
        );
        await checkRake(fixture, page, 0, light);
        if (light === 'rain' || light === 'snow') {
            await fixture.screenshot({
                path: `../../docs/leaf-rake-2026/${light}.png`,
            });
        } else {
...
test('leaf rake small low-quality canvas', async ({ mount, page }) => {
    const fixture = await mount(
        <LeafRakeFixture rotation={0} light="cloudy" small />,
    );
    await checkRake(fixture, page, 0);
    await expect(fixture).toHaveScreenshot('small.png');
});

for (const rotation of [0, 1, 2, 3]) {
    test(`leaf rake raised supports ${rotation}`, async ({ mount, page }) => {
        const fixture = await mount(
            <LeafRakeFixture rotation={rotation} raised />,
        );
        await checkRake(fixture, page, rotation, '', true);
    });
}
