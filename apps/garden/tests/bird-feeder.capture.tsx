...
import { BirdFeederFixture } from '../../../packages/game/tests/BirdFeederFixture';
...
async function checkFeeder(
    ...
    for (const item of data.clusters) {
        ...
        expect(item.triangles).toBe(496);
        expect(item.rotation).toBeCloseTo((rotation * Math.PI) / 2, 5);
        expect(item.minY).toBeCloseTo(raised ? 1.07 : 0.4, 3);
        expect(item.height).toBeGreaterThan(0.83);
        expect(item.height).toBeLessThanOrEqual(0.85);
...
for (const light of ['day', 'night'] satisfies ('day' | 'night')[]) {
    for (const rotation of [0, 1, 2, 3]) {
        test(`bird feeder ${light} ${rotation}`, async ({ mount, page }) => {
            const errors: string[] = [];
            page.on('pageerror', (error) => errors.push(error.message));
            const fixture = await mount(
                <BirdFeederFixture rotation={rotation} light={light} />,
            );
            await checkFeeder(fixture, page, rotation);
...
for (const light of ['cloudy', 'dusk', 'rain', 'snow'] satisfies (
    ...
    test(`bird feeder ${light}`, async ({ mount, page }) => {
        const fixture = await mount(
            <BirdFeederFixture rotation={0} light={light} />,
        );
        await checkFeeder(fixture, page, 0, light);
        if (light === 'rain' || light === 'snow') {
            await fixture.screenshot({
                path: `../../docs/bird-feeder-2026/${light}.png`,
            });
        } else {
...
test('bird feeder small low-quality canvas', async ({ mount, page }) => {
    const fixture = await mount(
        <BirdFeederFixture rotation={0} light="cloudy" small />,
    );
    await checkFeeder(fixture, page, 0);
    await expect(fixture).toHaveScreenshot('small.png');
});

for (const rotation of [0, 1, 2, 3]) {
    test(`bird feeder raised supports ${rotation}`, async ({ mount, page }) => {
        const fixture = await mount(
            <BirdFeederFixture rotation={rotation} raised />,
        );
        await checkFeeder(fixture, page, rotation, '', true);
    });
}
