...
import { FallenLogFixture } from '../../../packages/game/tests/FallenLogFixture';
...
async function checkLog(
    ...
    for (const item of data.clusters) {
        ...
        expect(item.triangles).toBe(389);
        ...
        expect(item.width).toBeLessThanOrEqual(rotation % 2 ? 0.9 : 1.9);
        expect(item.depth).toBeLessThanOrEqual(rotation % 2 ? 1.9 : 0.9);
...
for (const light of ['day', 'night'] satisfies ('day' | 'night')[]) {
    for (const rotation of [0, 1, 2, 3]) {
        test(`fallen log ${light} ${rotation}`, async ({ mount, page }) => {
            const errors: string[] = [];
            page.on('pageerror', (error) => errors.push(error.message));
            const fixture = await mount(
                <FallenLogFixture rotation={rotation} light={light} />,
            );
...
for (const light of ['cloudy', 'dusk', 'rain', 'snow'] satisfies (
    ...
    test(`fallen log ${light}`, async ({ mount, page }) => {
        const fixture = await mount(
            <FallenLogFixture rotation={0} light={light} />,
        );
        await checkLog(fixture, page, 0, light);
        if (light === 'rain' || light === 'snow') {
            await fixture.screenshot({
                path: `../../docs/fallen-log-2026/${light}.png`,
            });
        } else {
...
test('fallen log small low-quality canvas', async ({ mount, page }) => {
    const fixture = await mount(
        <FallenLogFixture rotation={0} light="cloudy" small />,
    );
...
for (const rotation of [0, 1, 2, 3]) {
    test(`fallen log raised supports ${rotation}`, async ({ mount, page }) => {
        const fixture = await mount(
            <FallenLogFixture rotation={rotation} raised />,
        );
