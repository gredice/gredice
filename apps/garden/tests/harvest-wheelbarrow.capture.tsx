...
import { HarvestWheelbarrowFixture } from '../../../packages/game/tests/HarvestWheelbarrowFixture';

for (const night of [false, true]) {
    for (const rotation of [0, 1, 2, 3]) {
        test(`wheelbarrow beside a planted bed: ${night ? 'night' : 'day'} ${rotation}`, async ({
            mount,
            page,
        }) => {
            const errors: string[] = [];
            ...
            const fixture = await mount(
                <HarvestWheelbarrowFixture rotation={rotation} night={night} />,
            );
            ...
            expect(data.height).toBeGreaterThan(0.88);
            expect(data.height).toBeLessThanOrEqual(0.89);
            expect(data.minX).toBeGreaterThanOrEqual(-2.45);
            expect(data.maxX).toBeLessThanOrEqual(
                -2 + (rotation % 2 ? 1 : 2) - 0.55,
            );
            expect(data.minZ).toBeGreaterThanOrEqual(-0.45);
            expect(data.maxZ).toBeLessThanOrEqual(
                (rotation % 2 ? 2 : 1) - 0.55,
            );
...
test('wheelbarrow and crop control remain readable on a small low-quality canvas', async ({
    mount,
    page,
}) => {
    const fixture = await mount(
        <HarvestWheelbarrowFixture rotation={0} small />,
    );
    await expect(fixture).toHaveAttribute('data-ready', /.+/);
