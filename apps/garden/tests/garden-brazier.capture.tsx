...
import { GardenBrazierFixture } from '../../../packages/game/tests/GardenBrazierFixture';

async function checkBrazier(
    fixture: Locator,
    page: Page,
    rotation: number,
    weather = '',
    ...
    const data: {
        ...
        clusters: {
            ...
            ownsEmberMaterial: boolean;
            sharesEmberGeometry: boolean;
            sourceEmissive: number;
    ...
    for (const item of data.clusters) {
        ...
        expect(item.emberIntensity).toBe(0);
        expect(item.ownsEmberMaterial).toBe(true);
        expect(item.sharesEmberGeometry).toBe(true);
        expect(item.sourceEmissive).toBeGreaterThan(0);
        expect(item.colorCount).toBeGreaterThan(0);
        expect(item.triangles).toBe(1088);
        expect(item.rotation).toBeCloseTo((rotation * Math.PI) / 2, 5);
        expect(item.minY).toBeCloseTo(raised ? 1.07 : 0.4, 3);
        expect(item.height).toBeGreaterThan(0.48);
        expect(item.height).toBeLessThanOrEqual(0.5);
...
for (const light of ['day', 'night'] satisfies ('day' | 'night')[]) {
    for (const rotation of [0, 1, 2, 3]) {
        test(`garden brazier ${light} ${rotation}`, async ({ mount, page }) => {
            const errors: string[] = [];
            page.on('pageerror', (error) => errors.push(error.message));
            const fixture = await mount(
                <GardenBrazierFixture rotation={rotation} light={light} />,
            );
...
for (const light of ['cloudy', 'dusk', 'rain', 'snow'] satisfies (
    ...
    test(`garden brazier ${light}`, async ({ mount, page }) => {
        const fixture = await mount(
            <GardenBrazierFixture rotation={0} light={light} />,
        );
        await checkBrazier(fixture, page, 0, light);
...
test('garden brazier small low-quality canvas', async ({ mount, page }) => {
    const fixture = await mount(
...
for (const rotation of [0, 1, 2, 3]) {
    test(`garden brazier raised supports ${rotation}`, async ({
        mount,
        page,
    }) => {
        const fixture = await mount(
            <GardenBrazierFixture rotation={rotation} raised />,
        );
        await checkBrazier(fixture, page, rotation, '', true);
    });
}

test('garden brazier remains unlit with weather disabled', async ({
    mount,
    page,
}) => {
    const fixture = await mount(
        <GardenBrazierFixture rotation={0} light="rain" weatherDisabled />,
    );
    await checkBrazier(fixture, page, 0);
    const data: { clusters: { snow: number; rain: number }[] } = JSON.parse(
        (await fixture.getAttribute('data-ready')) ?? '{}',
    );
    expect(data.clusters.map(({ snow, rain }) => ({ snow, rain }))).toEqual([
        { snow: 0, rain: 0 },
    ]);
});
