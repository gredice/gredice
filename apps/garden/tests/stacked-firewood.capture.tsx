...
import { StackedFirewoodFixture } from '../../../packages/game/tests/StackedFirewoodFixture';

async function checkFirewood(
    fixture: Locator,
    page: Page,
    rotation: number,
    weather = '',
    ...
    for (const item of data.clusters) {
        ...
        expect(item.triangles).toBe(408);
...
for (const light of ['day', 'night'] satisfies ('day' | 'night')[]) {
    for (const rotation of [0, 1, 2, 3]) {
        test(`stacked firewood ${light} ${rotation}`, async ({
            mount,
            page,
        }) => {
            const errors: string[] = [];
            ...
            const fixture = await mount(
                <StackedFirewoodFixture rotation={rotation} light={light} />,
            );
            await checkFirewood(fixture, page, rotation);
...
for (const light of ['cloudy', 'dusk', 'rain', 'snow'] satisfies (
    ...
    test(`stacked firewood ${light}`, async ({ mount, page }) => {
        const fixture = await mount(
            <StackedFirewoodFixture rotation={0} light={light} />,
        );
        await checkFirewood(fixture, page, 0, light);
        if (light === 'rain' || light === 'snow') {
            await fixture.screenshot({
                path: `../../docs/stacked-firewood-2026/${light}.png`,
            });
        } else {
...
test('stacked firewood small low-quality canvas', async ({ mount, page }) => {
    const fixture = await mount(
        <StackedFirewoodFixture rotation={0} light="cloudy" small />,
    );
    await checkFirewood(fixture, page, 0);
    await expect(fixture).toHaveScreenshot('small.png');
});

for (const rotation of [0, 1, 2, 3]) {
    test(`stacked firewood raised supports ${rotation}`, async ({
        mount,
        page,
    }) => {
        const fixture = await mount(
            <StackedFirewoodFixture rotation={rotation} raised />,
        );
        await checkFirewood(fixture, page, rotation, '', true);
    });
}
