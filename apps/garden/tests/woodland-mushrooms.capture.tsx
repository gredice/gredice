...
import { WoodlandMushroomsFixture } from '../../../packages/game/tests/WoodlandMushroomsFixture';
...
async function checkClusters(
    ...
    for (const item of data.clusters) {
        ...
        expect(item.triangles).toBe(1160);
        expect(item.rotation).toBeCloseTo((rotation * Math.PI) / 2, 5);
        expect(item.minY).toBeCloseTo(
            item.id === 'mushrooms-table' ? 1.07 : 0.4,
            3,
        );
        expect(item.height).toBeGreaterThan(0.37);
        expect(item.height).toBeLessThanOrEqual(0.4);
...
for (const light of ['day', 'night'] satisfies ('day' | 'night')[]) {
    for (const rotation of [0, 1, 2, 3]) {
        test(`woodland mushrooms ${light} ${rotation}`, async ({
            mount,
            page,
        }) => {
            const errors: string[] = [];
            ...
            const fixture = await mount(
                <WoodlandMushroomsFixture rotation={rotation} light={light} />,
            );
...
for (const light of ['cloudy', 'dusk', 'rain', 'snow'] satisfies (
    ...
    test(`woodland mushrooms ${light}`, async ({ mount, page }) => {
        const fixture = await mount(
            <WoodlandMushroomsFixture rotation={0} light={light} />,
        );
        await checkClusters(fixture, page, 0, light);
        if (light === 'rain' || light === 'snow') {
            await fixture.screenshot({
                path: `../../docs/woodland-mushrooms-2026/${light}.png`,
            });
        } else {
...
test('woodland mushrooms small low-quality canvas', async ({ mount, page }) => {
    const fixture = await mount(
        <WoodlandMushroomsFixture rotation={0} light="cloudy" small />,
    );
