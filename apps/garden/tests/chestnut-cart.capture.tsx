...
async function checkCart(
    ...
    for (const item of data.clusters) {
        ...
        expect(item.triangles).toBe(2016);
        expect(item.rotation).toBeCloseTo((rotation * Math.PI) / 2, 5);
        expect(item.minY).toBeCloseTo(raised ? 1.07 : 0.4, 3);
        expect(item.height).toBeGreaterThan(1.41);
        expect(item.height).toBeLessThanOrEqual(1.45);
        expect(item.width).toBeLessThanOrEqual(rotation % 2 ? 0.8 : 1.6);
        expect(item.depth).toBeLessThanOrEqual(rotation % 2 ? 1.6 : 0.8);
...
for (const light of ['day', 'night'] satisfies ('day' | 'night')[]) {
    for (const rotation of [0, 1, 2, 3]) {
        test(`chestnut cart ${light} ${rotation}`, async ({ mount, page }) => {
            const errors: string[] = [];
            ...
            await checkCart(fixture, page, rotation);
...
for (const light of ['cloudy', 'dusk', 'rain', 'snow'] satisfies (
    ...
    test(`chestnut cart ${light}`, async ({ mount, page }) => {
        const fixture = await mount(
            <ChestnutRoastingCartFixture rotation={0} light={light} />,
        );
        await checkCart(fixture, page, 0, light);
        if (light === 'rain' || light === 'snow') {
            await fixture.screenshot({
                path: `../../docs/chestnut-cart-2026/${light}.png`,
            });
        } else {
...
test('chestnut cart small low-quality canvas', async ({ mount, page }) => {
    const fixture = await mount(
        <ChestnutRoastingCartFixture rotation={0} light="cloudy" small />,
    );
    await checkCart(fixture, page, 0);
    await expect(fixture).toHaveScreenshot('small.png');
});

for (const rotation of [0, 1, 2, 3]) {
    test(`chestnut cart raised supports ${rotation}`, async ({
        mount,
        page,
    }) => {
        const fixture = await mount(
            <ChestnutRoastingCartFixture rotation={rotation} raised />,
        );
        await checkCart(fixture, page, rotation, '', true);
    });
}
