...
import { SeedDryingRackFixture } from '../../../packages/game/tests/SeedDryingRackFixture';
...
for (const light of ['day', 'night'] satisfies ('day' | 'night')[]) {
    for (const rotation of [0, 1, 2, 3]) {
        test(`seed drying rack ${light} ${rotation}`, async ({
            mount,
            page,
        }) => {
            const errors: string[] = [];
            ...
            const fixture = await mount(
                <SeedDryingRackFixture rotation={rotation} light={light} />,
            );
            await checkRack(fixture, page, rotation);
...
for (const light of ['cloudy', 'dusk', 'rain', 'snow'] satisfies (
    ...
    test(`seed drying rack ${light}`, async ({ mount, page }) => {
        const fixture = await mount(
            <SeedDryingRackFixture rotation={0} light={light} />,
        );
        await checkRack(fixture, page, 0, light);
        if (light === 'rain' || light === 'snow') {
            await fixture.screenshot({
                path: `../../docs/seed-drying-rack-2026/${light}.png`,
            });
        } else {
...
test('seed drying rack small low-quality canvas', async ({ mount, page }) => {
    const fixture = await mount(
        <SeedDryingRackFixture rotation={0} light="cloudy" small />,
    );
    await checkRack(fixture, page, 0);
    await expect(fixture).toHaveScreenshot('small.png');
});

for (const rotation of [0, 1, 2, 3]) {
    test(`seed drying rack raised supports ${rotation}`, async ({
        mount,
        page,
    }) => {
        const fixture = await mount(
            <SeedDryingRackFixture rotation={rotation} raised />,
        );
        await checkRack(fixture, page, rotation, '', true);
    });
}
