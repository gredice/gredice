...
export type FeedbackModalProps = {
    title?: string;
    topic: string;
    data?: Record<string, string | string[] | number | number[] | boolean | null | undefined>;
} & Omit<FeedbackTriggerProps, 'onFeedback' | 'onClick'>;

export function FeedbackModal({ title, topic, data, ...rest }: FeedbackModalProps) {
    const [open, setOpen] = useState(false);
    const [score, setScore] = useState<'like' | 'dislike' | 'neutral' | null>(null);
    ...
    async function handleFeedback(formData: FormData) {
        ...
        await client().api.feedback.$post({
            json: {
                topic,
                data,
                score: (score === 'like' ? 1 : score === 'dislike' ? -1 : 0).toString(),
                comment
            }
        });
        ...
        <Popper
            ...
            <form action={handleFeedback}>
                <Stack spacing={2} className="p-4">
                    <Typography level="body1" secondary>Kako ti se sviđa ovaj sadržaj?</Typography>
                    ...
                    <Button variant="outlined" type="submit" endDecorator={<Send className="size-4" />}>Pošalji</Button>
                </Stack>
            </form>
        </Popper>
    );
