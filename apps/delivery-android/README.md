...
The launcher is a small native pairing/logout shell; the operational phone UI remains the Delivery TWA. Pairing uses the system browser, the fixed `gredice-delivery-android` public client, S256 PKCE, and the exact verified callback `/android/auth/callback`. Only the rotating refresh credential is persisted, encrypted by an Android Keystore AES-GCM key and excluded from backup. Access credentials and the active-route cache remain process memory only.

The car display fails closed with **Prijavite se u aplikaciji na telefonu** when no native session is available. Refresh is serialized across the process, one failed API request is retried after rotation, and a second `401` clears the local session and route cache.
