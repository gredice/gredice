...
async function ensureHostsEntries() {
    ...
    if (missingHosts.length === 0) {
        console.log('Verified hosts file entries for the local gredice.test domains.');
        return;
    }
...
async function main() {
    if (shouldSkipProxy) {
        ...
        try {
            await startProxy();
            console.log('Apps will be available on the local gredice.test domains once Turbo finishes booting.');
