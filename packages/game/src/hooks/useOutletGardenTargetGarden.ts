...
export function useOutletGardenTargetGarden(gardenId: number | null) {
    return useQuery({
        ...
        queryFn: () => fetchGardenTarget(gardenId),
...
export async function fetchGardenTarget(gardenId: number | null) {
    if (gardenId === null) {
        return null;
    }

    const response = await clientAuthenticated().api.gardens[':gardenId'].$get({
        param: { gardenId: gardenId.toString() },
    });
    if (response.status === 401) {
        throw new OutletGardenAuthenticationRequiredError(
            'Garden target authentication expired',
        );
    }
    ...
    if (response.status !== 200) {
        throw new Error('Failed to fetch garden target');
    }
