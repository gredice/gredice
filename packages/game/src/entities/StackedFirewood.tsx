...
export function StackedFirewood({
    stack,
    block,
    rotation,
    weatherDisabled,
}: EntityInstanceProps) {
    const { nodes } = useGameGLTF('StackedFirewood');
    ...
    return (
        <animated.group
            name={`StackedFirewood:${block.id}`}
            position={stack.position.clone().setY(height)}
            rotation-y={animatedRotation?.to((_, y) => y)}
        >
            <WeatheredEntityPart
                node={nodes.StackedFirewood_Timber}
                material={nodes.StackedFirewood_Timber.material}
                snow={
                    disabled
                        ? false
