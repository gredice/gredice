...
export function SeedDryingRack({
    stack,
    block,
    rotation,
    weatherDisabled,
}: EntityInstanceProps) {
    const { nodes } = useGameGLTF('SeedDryingRack');
    ...
    return (
        <animated.group
            name={`SeedDryingRack:${block.id}`}
            position={stack.position.clone().setY(height)}
            rotation-y={animatedRotation?.to((_, y) => y)}
        >
            <WeatheredEntityPart
                node={nodes.SeedDryingRack_Arrangement}
                material={nodes.SeedDryingRack_Arrangement.material}
                snow={
                    disabled
                        ? false
