...
import releaseManifest from '../../../../docs/autumn-leaf-piles-2026/release-manifest.json';
...
describe('Autumn leaf pile release', () => {
    ...
    it('keeps each pile placeable on supports, but never supports another block', () => {
        assert.equal(new Set(autumnLeafPiles.map((item) => item.name)).size, 2);
        const blocks = getLocalSandboxBlockData();
        for (const item of autumnLeafPiles) {
            ...
            assert.ok(
                block.information.label.startsWith('Ukrasno jesensko lišće'),
            );
            assert.match(
    ...
    for (const asset of autumnLeafPileNames) {
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            assert.equal(
                triangles,
                asset === 'AutumnLeafPileMound' ? 198 : 248,
            );
            ...
            for (const item of autumnLeafPiles.filter(
                ...
                for (let rotation = 0; rotation < 4; rotation++) {
                    ...
                    const anchor = new Vector3(
                        ...autumnLeafPileInteractionAnchors[asset],
                    ).applyAxisAngle(
                        new Vector3(0, 1, 0),
                        (rotation * Math.PI) / 2,
                    );
                    ...
                    assert.ok(
                        ...
                        'The activity anchor must remain selectable from the default camera after rotation',
                    );
