...
import releaseManifest from '../../../../docs/autumn-blanket-bench-2026/release-manifest.json';
...
const benches = [autumnBlanketBench];

describe('AutumnBlanketBench release', () => {
    ...
    it('keeps the bench placeable on supports, but never supports another block', () => {
        assert.equal(new Set(benches.map((item) => item.name)).size, 1);
        ...
        for (const item of benches) {
            ...
            assert.equal(
                ...
                'Drvena klupa s jesenskom dekom',
            );
