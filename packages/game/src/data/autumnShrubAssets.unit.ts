...
import releaseManifest from '../../../../docs/autumn-shrub-2026/release-manifest.json';
...
const autumnShrubs = [autumnShrub];

describe('Deciduous autumn shrub release', () => {
    ...
    it('keeps the shrub placeable on supports, but never supports another block', () => {
        assert.equal(new Set(autumnShrubs.map((item) => item.name)).size, 1);
        const blocks = getLocalSandboxBlockData();
        for (const item of autumnShrubs) {
            ...
            assert.ok(
                block.information.label.startsWith('Listopadni ukrasni grm'),
            );
    ...
    for (const asset of [
        ...
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            assert.equal(document.meshes.length, 7);
