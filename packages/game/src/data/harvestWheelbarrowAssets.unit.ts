...
import releaseManifest from '../../../../docs/harvest-wheelbarrow-2026/release-manifest.json';
...
const wheelbarrows = [harvestWheelbarrow];

describe('HarvestWheelbarrow release', () => {
    ...
    it('keeps the wheelbarrow placeable on supports, but never supports another block', () => {
        assert.equal(new Set(wheelbarrows.map((item) => item.name)).size, 1);
        const blocks = getLocalSandboxBlockData();
        for (const item of wheelbarrows) {
            ...
            assert.equal(block.information.label, 'Ukrasna kolica s bundevama');
    ...
    for (const asset of [
        ...
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            assert.equal(document.meshes.length, 6);
