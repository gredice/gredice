...
import releaseManifest from '../../../../docs/leaf-rake-2026/release-manifest.json';
...
const rakeItems = [leafRake];

describe('Leaf rake release', () => {
    ...
    it('keeps the rake placeable on supports, but never supports another block', () => {
        assert.equal(new Set(rakeItems.map((item) => item.name)).size, 1);
        const blocks = getLocalSandboxBlockData();
        for (const item of rakeItems) {
            ...
            assert.ok(
                block.information.label.startsWith('Grablje s hrpom lišća'),
            );
            assert.match(
                item.information.fullDescription,
                /ne naručuje niti potvrđuje/,
            );
    ...
    for (const asset of [
        ...
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            assert.equal(triangles, 590);
            assert.ok(
                palette.size >= 8,
                'Leaf, wood and metal colours share one material',
            );
