...
import releaseManifest from '../../../../docs/garden-scarecrow-2026/release-manifest.json';
...
const scarecrows = [gardenScarecrow];

describe('GardenScarecrow release', () => {
    ...
    it('keeps the scarecrow placeable on supports, but never supports another block', () => {
        assert.equal(new Set(scarecrows.map((item) => item.name)).size, 1);
        const blocks = getLocalSandboxBlockData();
        for (const item of scarecrows) {
            ...
            assert.ok(block.information.label === 'Vrtno strašilo');
