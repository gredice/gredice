...
import {
    ...
    woodlandArrangements,
    woodlandObservationAnchors,
    woodlandSelectionAnchors,
} from '@gredice/js/woodlandArrangements';

import { Box3, Mesh, Raycaster, Vector3 } from 'three';
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js';
import releaseManifest from '../../../../docs/woodland-arrangements-2026/release-manifest.json';
...
describe('Woodland arrangement release', () => {
    ...
    it('keeps each woodland arrangement placeable on supports, but never supports another block', () => {
        assert.equal(
            new Set(woodlandArrangements.map((item) => item.name)).size,
            3,
        );
        const blocks = getLocalSandboxBlockData();
        for (const item of woodlandArrangements) {
            ...
            assert.ok(block.information.label.startsWith('Šumski ukras'));
            assert.match(
                item.information.fullDescription,
                /ne predstavlja jestivu vrstu/,
            );
    ...
    for (const asset of woodlandArrangementNames) {
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            const observation = gltf.scene.getObjectByName(
                `${asset}_Observation`,
            );
            ...
            assert.ok(
                observation
                    ...
                    .distanceTo(
                        new Vector3(...woodlandObservationAnchors[asset]),
                    ) < 0.00001,
            );
            let triangles = 0;
            ...
            assert.equal(
                triangles,
                asset === 'WoodlandAcorns'
                    ? 620
                    : asset === 'WoodlandConkers'
                      ? 486
                      : 960,
            );
            ...
            for (const item of woodlandArrangements.filter(
                ...
                for (let rotation = 0; rotation < 4; rotation++) {
                    ...
                    const anchor = new Vector3(
                        ...woodlandSelectionAnchors[asset],
                    ).applyAxisAngle(
                        new Vector3(0, 1, 0),
                        (rotation * Math.PI) / 2,
                    );
                    ...
                    assert.ok(
                        ...
                        'The arrangement selection point must remain selectable from the default camera after rotation',
                    );
