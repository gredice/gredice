...
import releaseManifest from '../../../../docs/seed-drying-rack-2026/release-manifest.json';
...
const rackItems = [seedDryingRack];

describe('Seed drying rack release', () => {
    ...
    it('keeps the rack placeable on supports, but never supports another block', () => {
        assert.equal(new Set(rackItems.map((item) => item.name)).size, 1);
        ...
        for (const item of rackItems) {
            ...
            assert.ok(
                block.information.label.startsWith(
                    'Ukrasni stalak za sušenje sjemena',
                ),
            );
            assert.match(
                item.information.fullDescription,
                /ne mijenja zalihu sjemena/,
            );
    ...
    for (const asset of [
        ...
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            assert.equal(triangles, 1452);
            assert.ok(
                palette.size >= 8,
                'Timber, tray and dry seed-head colours share one material',
            );
