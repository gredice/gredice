...
import { harvestCrateNames, harvestCrates } from '@gredice/js/harvestCrates';

import { Box3, Mesh, Vector3 } from 'three';
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js';
import releaseManifest from '../../../../docs/harvest-crates-2026/release-manifest.json';
...
describe('Harvest crate release', () => {
    ...
    it('keeps each crate placeable on supports, but never supports another block', () => {
        assert.equal(new Set(harvestCrates.map((item) => item.name)).size, 2);
        ...
        for (const item of harvestCrates) {
            ...
            assert.ok(block.information.label.startsWith('Ukrasni sanduk'));
            assert.match(
                item.information.fullDescription,
                /ne prikazuje stvarni urod/,
            );
    ...
    for (const asset of harvestCrateNames) {
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            assert.equal(document.meshes.length, 5);
            ...
            assert.equal(triangles, asset === 'HarvestCrate' ? 3204 : 2132);
            ...
            for (const item of harvestCrates.filter(
