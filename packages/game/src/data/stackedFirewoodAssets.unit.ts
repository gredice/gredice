...
import releaseManifest from '../../../../docs/stacked-firewood-2026/release-manifest.json';
...
const woodItems = [stackedFirewood];

describe('Stacked firewood release', () => {
    ...
    it('keeps the wood stack placeable on supports, but never supports another block', () => {
        assert.equal(new Set(woodItems.map((item) => item.name)).size, 1);
        ...
        for (const item of woodItems) {
            ...
            assert.ok(
                block.information.label.startsWith('Složena drva za ogrjev'),
            );
            assert.match(item.information.fullDescription, /ne daju resurse/);
    ...
    for (const asset of [
        ...
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            assert.equal(triangles, 408);
            assert.ok(
                palette.size >= 8,
                'Bark, cut ends and split face colours share one material',
            );
