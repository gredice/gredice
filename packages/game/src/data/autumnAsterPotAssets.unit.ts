...
import releaseManifest from '../../../../docs/autumn-aster-pots-2026/release-manifest.json';
...
describe('Autumn aster pot release', () => {
    ...
    it('keeps each aster pot placeable on supports, but never supports another block', () => {
        assert.equal(new Set(autumnAsterPots.map((item) => item.name)).size, 3);
        ...
        for (const item of autumnAsterPots) {
            ...
            assert.ok(
                block.information.label.startsWith('Ukrasni jesenski asteri'),
            );
            assert.match(
                item.information.fullDescription,
                /ne siju se, ne rastu i ne daju urod/,
            );
