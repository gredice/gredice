...
import releaseManifest from '../../../../docs/woodland-mushrooms-2026/release-manifest.json';
...
const mushroomItems = [woodlandMushrooms];

describe('Woodland mushrooms release', () => {
    ...
    it('keeps the mushrooms placeable on supports, but never supports another block', () => {
        assert.equal(new Set(mushroomItems.map((item) => item.name)).size, 1);
        const blocks = getLocalSandboxBlockData();
        for (const item of mushroomItems) {
            ...
            assert.ok(
                block.information.label.startsWith('Ukrasne šumske gljive'),
            );
            assert.match(item.information.fullDescription, /ne bere se/);
    ...
    for (const asset of [
        ...
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            assert.equal(triangles, 1160);
            assert.ok(
                palette.size >= 8,
                'Cap, rim and stem colours share one material',
            );
