...
import releaseManifest from '../../../../docs/hedgehog-shelter-2026/release-manifest.json';
...
const shelterItems = [hedgehogShelter];

describe('Hedgehog shelter release', () => {
    ...
    it('keeps the shelter placeable on supports, but never supports another block', () => {
        assert.equal(new Set(shelterItems.map((item) => item.name)).size, 1);
        const blocks = getLocalSandboxBlockData();
        for (const item of shelterItems) {
            ...
            assert.ok(block.information.label.startsWith('Sklonište za ježa'));
            assert.match(item.information.fullDescription, /Ne traži hranu/);
    ...
    for (const asset of [
        ...
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            assert.equal(triangles, 260);
            assert.ok(
                palette.size >= 4,
                'Roof, supports and bedding share one opaque palette',
            );
