...
import releaseManifest from '../../../../docs/chestnut-cart-2026/release-manifest.json';
...
const carts = [chestnutRoastingCart];

describe('ChestnutRoastingCart release', () => {
    ...
    it('keeps the cart placeable on supports, but never supports another block', () => {
        assert.equal(new Set(carts.map((item) => item.name)).size, 1);
        ...
        for (const item of carts) {
            ...
            assert.equal(block.information.label, 'Kolica s pečenim kestenima');
    ...
    for (const asset of [
        ...
        it(`${asset} has versioned lazy geometry inside every rotation's declared hitbox`, async () => {
            ...
            assert.equal(triangles, 2016);
            assert.ok(bytes.length < 300_000);
            gltf.scene.updateMatrixWorld(true);
            for (const anchor of chestnutRoastingCartEffectAnchors) {
                const authored = gltf.scene.getObjectByName(
                    `ChestnutRoastingCart_${anchor.id[0].toUpperCase()}${anchor.id.slice(1)}`,
                );
                assert.ok(authored);
                assert.ok(
                    authored
