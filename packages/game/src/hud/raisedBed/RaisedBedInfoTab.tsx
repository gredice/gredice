...
export function RaisedBedInfoTab({
    ...
    const yieldStats = raisedBed?.fields.reduce(
        (acc, field) => {
            ...
            const plantYieldMin =
                sortData.information.plant.attributes?.yieldMin ?? 0;
            const plantYieldMax =
                sortData.information.plant.attributes?.yieldMax ?? 0;
            ...
            <Stack>
                ...
                <Stack>
                    ...
                    <Typography level="body2">
                        {(yieldStats.min / 1000).toFixed(2)} kg -{' '}
                        {(yieldStats.max / 1000).toFixed(2)} kg
                    </Typography>
                </Stack>
            </Stack>
        </div>
