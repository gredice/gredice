...
const models = autumnGrasses.map(({ name }) => {
...
const release = {
    ...
    sources: autumnGrasses.map(({ name }) =>
        file(`assets/game-assets/${name}.blend`),
    ),
    models,
    items: autumnGrasses.map((item) => {
...
mkdirSync(new URL('docs/autumn-grasses-2026/', root), { recursive: true });
writeFileSync(
    new URL('docs/autumn-grasses-2026/release-manifest.json', root),
