...
const models = halloweenAccents.map(({ name }) => {
...
const release = {
    ...
    sources: halloweenAccents.map(({ name }) =>
        file(`assets/game-assets/${name}.blend`),
    ),
    models,
    items: halloweenAccents.map((item) => {
...
mkdirSync(new URL('docs/halloween-accents-2026/', root), {
    recursive: true,
});
writeFileSync(
    new URL('docs/halloween-accents-2026/release-manifest.json', root),
