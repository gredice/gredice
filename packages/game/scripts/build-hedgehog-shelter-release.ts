...
const release = {
    ...
    sources: modelNames.map((name) => file(`assets/game-assets/${name}.blend`)),
    models,
    items: [hedgehogShelter].map((item) => {
...
mkdirSync(new URL('docs/hedgehog-shelter-2026/', root), {
    recursive: true,
});
writeFileSync(
    new URL('docs/hedgehog-shelter-2026/release-manifest.json', root),
