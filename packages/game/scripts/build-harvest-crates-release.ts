...
const models = harvestCrates.map(({ name }) => {
...
const release = {
    ...
    sources: harvestCrates.map(({ name }) =>
        file(`assets/game-assets/${name}.blend`),
    ),
    models,
    items: harvestCrates.map((item) => {
...
mkdirSync(new URL('docs/harvest-crates-2026/', root), { recursive: true });
writeFileSync(
    new URL('docs/harvest-crates-2026/release-manifest.json', root),
