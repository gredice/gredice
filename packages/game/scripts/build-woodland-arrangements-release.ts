...
const models = woodlandArrangements.map(({ name }) => {
...
const release = {
    ...
    sources: woodlandArrangements.map(({ name }) =>
        file(`assets/game-assets/${name}.blend`),
    ),
    models,
    items: woodlandArrangements.map((item) => {
...
mkdirSync(new URL('docs/woodland-arrangements-2026/', root), {
    recursive: true,
});
writeFileSync(
    new URL('docs/woodland-arrangements-2026/release-manifest.json', root),
