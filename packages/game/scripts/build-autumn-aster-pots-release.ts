...
const models = (
    ['AutumnAsterPot'] satisfies (keyof typeof gameAssetModels)[]
).map((name) => {
...
const release = {
    ...
    sources: [file('assets/game-assets/AutumnAsterPot.blend')],
    reusedPot: file('assets/game-assets/PotLowBowl.blend'),
    models,
    items: autumnAsterPots.map((item) => {
...
mkdirSync(new URL('docs/autumn-aster-pots-2026/', root), { recursive: true });
writeFileSync(
    new URL('docs/autumn-aster-pots-2026/release-manifest.json', root),
