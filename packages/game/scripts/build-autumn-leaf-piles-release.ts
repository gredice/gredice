...
const models = autumnLeafPiles.map(({ name }) => {
...
const release = {
    ...
    sources: autumnLeafPiles.map(({ name }) =>
        file(`assets/game-assets/${name}.blend`),
    ),
    models,
    items: autumnLeafPiles.map((item) => {
...
mkdirSync(new URL('docs/autumn-leaf-piles-2026/', root), { recursive: true });
writeFileSync(
    new URL('docs/autumn-leaf-piles-2026/release-manifest.json', root),
