...
const models = autumnEntrances.map(({ name }) => {
...
const release = {
    ...
    sources: autumnEntrances.map(({ name }) =>
        file(`assets/game-assets/${name}.blend`),
    ),
    models,
    items: autumnEntrances.map((item) => {
...
mkdirSync(new URL('docs/autumn-entrances-2026/', root), {
    recursive: true,
});
writeFileSync(
    new URL('docs/autumn-entrances-2026/release-manifest.json', root),
