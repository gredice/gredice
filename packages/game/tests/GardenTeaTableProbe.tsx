...
export function GardenTeaTableProbe({
    onReady,
    weather,
}: {
    ...
    useSceneTimeInvalidation('test:garden-tea-table-ready', !reported.current);
    useLayoutEffect(() => {
    ...
    useFrame(() => {
        ...
        const clusters = ['tea-table'].map((id) => {
            const root = scene.getObjectByName(`GardenTeaTable:${id}`);
            const mesh = root?.getObjectByName('GardenTeaTable_Timber');
            const tea = root?.getObjectByName('GardenTeaTable_TeaSet');
            if (
                !root ||
                !(tea instanceof Mesh) ||
                ...
                throw new Error('Missing tea table geometry/material');
            const bounds = new Box3().setFromObject(root);
            // The central tabletop is a stable real-geometry ray target in every rotation.
            const point = root
                .localToWorld(new Vector3(0, 0.6432, 0))
                .project(camera);
            let snow = 0;
            let rain = 0;
            root.traverse((node) => {
            ...
            return {
                ...
                triangles:
                    (mesh.geometry.index?.count ??
                        ...
                        3 +
                    (tea.geometry.index?.count ??
                        tea.geometry.getAttribute('position').count) /
                        3,
                snow,
                rain,
