...
export function WoodlandArrangementsProbe({
    onReady,
    weather,
}: {
    ...
    useSceneTimeInvalidation(
        'test:woodland-arrangements-ready',
        !reported.current,
    );
    useLayoutEffect(() => {
    ...
    useFrame(() => {
        ...
        const clusters = ['acorns', 'conkers', 'basket'].map((id) => {
            const root = scene.getObjectByName(`WoodlandArrangement:${id}`);
            const mesh = root?.getObjectByName(
                id === 'acorns'
                    ? 'WoodlandAcorns_Arrangement'
                    : id === 'conkers'
                      ? 'WoodlandConkers_Arrangement'
                      : 'WoodlandMushroomBasket_Arrangement',
            );
            if (
                !root ||
                ...
                throw new Error(
                    'Missing woodland arrangement geometry/material',
                );
            const bounds = new Box3().setFromObject(root);
            const anchor = root.getObjectByName(
                `WoodlandArrangement:observation:${id}`,
            );
            if (!anchor) throw new Error('Missing stable activity anchor');
            const point = root
                .localToWorld(new Vector3(0, id === 'basket' ? 0.62 : 0.027, 0))
                .project(camera);
            let snow = 0;
            let rain = 0;
            root.traverse((node) => {
