...
export function SeedDryingRackProbe({
    onReady,
    weather,
}: {
    ...
    useSceneTimeInvalidation('test:seed-drying-rack-ready', !reported.current);
    useLayoutEffect(() => {
    ...
    useFrame(() => {
        ...
        const clusters = ['rack'].map((id) => {
            const root = scene.getObjectByName(`SeedDryingRack:${id}`);
            const mesh = root?.getObjectByName('SeedDryingRack_Arrangement');
            if (
                !root ||
                !(mesh instanceof Mesh) ||
                !(mesh.material instanceof MeshStandardMaterial)
            )
                throw new Error('Missing seed drying rack geometry/material');
            const bounds = new Box3().setFromObject(root);
            // The solid top hanging rail is a stable real-geometry ray target in every rotation.
            const point = root
                .localToWorld(new Vector3(0, 0.935, 0))
                .project(camera);
            let snow = 0;
            let rain = 0;
            root.traverse((node) => {
