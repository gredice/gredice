...
export function BirdFeederProbe({
    onReady,
    weather,
}: {
    ...
    useSceneTimeInvalidation('test:bird-feeder-ready', !reported.current);
    useLayoutEffect(() => {
    ...
    useFrame(() => {
        ...
        const clusters = ['feeder'].map((id) => {
            const root = scene.getObjectByName(`BirdFeeder:${id}`);
            const mesh = root?.getObjectByName('BirdFeeder_Arrangement');
            if (
                !root ||
            ...
            const point = root
                .localToWorld(new Vector3(0, 0.79, 0))
                .project(camera);
            let snow = 0;
            let rain = 0;
            root.traverse((node) => {
