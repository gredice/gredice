...
export function StackedFirewoodProbe({
    onReady,
    weather,
}: {
    ...
    useSceneTimeInvalidation('test:stacked-firewood-ready', !reported.current);
    useLayoutEffect(() => {
    ...
    useFrame(() => {
        ...
        const clusters = ['firewood'].map((id) => {
            const root = scene.getObjectByName(`StackedFirewood:${id}`);
            const mesh = root?.getObjectByName('StackedFirewood_Timber');
            if (
                !root ||
                !(mesh instanceof Mesh) ||
                !(mesh.material instanceof MeshStandardMaterial)
            )
                throw new Error('Missing stacked firewood geometry/material');
            const bounds = new Box3().setFromObject(root);
            // The broad upper split face is a stable real-geometry ray target in every rotation.
            const point = root
                .localToWorld(new Vector3(0, 0.425, 0))
                .project(camera);
            let snow = 0;
            let rain = 0;
            root.traverse((node) => {
