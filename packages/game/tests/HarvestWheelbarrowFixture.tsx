...
function WheelbarrowProbe({ onReady }: { onReady: (value: string) => void }) {
    const { scene, camera, size } = useThree();
    const reported = useRef(false);
    useSceneTimeInvalidation('test:wheelbarrow-ready', !reported.current);
    useLayoutEffect(() => {
    ...
    useFrame(() => {
        ...
        const object = scene.getObjectByName('HarvestWheelbarrow:wheelbarrow');
        const load = object?.getObjectByName('HarvestWheelbarrow_Pumpkins');
        if (!object || !load)
            throw new Error('Missing wheelbarrow runtime geometry');
...
export function HarvestWheelbarrowFixture({
    rotation,
    night = false,
    small = false,
}: {
                ...
                <GameStateContext.Provider value={store}>
                    ...
                    >
                        ...
                        The full close-up HUD is DOM above the scene and has separate app tests. */}
                                ...
                                <WheelbarrowProbe onReady={setReady} />
                            </Suspense>
                        </Scene>
                    </div>
