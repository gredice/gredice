...
export function LeafRakeProbe({
    ...
    useSceneTimeInvalidation('test:leaf-rake-ready', !reported.current);
    useLayoutEffect(() => {
    ...
    useFrame(() => {
        ...
        const clusters = ['rake'].map((id) => {
            const root = scene.getObjectByName(`LeafRake:${id}`);
            const mesh = root?.getObjectByName('LeafRake_Arrangement');
            if (
                !root ||
                !(mesh instanceof Mesh) ||
                !(mesh.material instanceof MeshStandardMaterial)
            )
                throw new Error('Missing leaf rake geometry/material');
            const bounds = new Box3().setFromObject(root);
            // The broad leaf mound is a stable real-geometry ray target in every rotation.
            const point = root
                .localToWorld(new Vector3(0, 0.08, 0))
                .project(camera);
            let snow = 0;
            let rain = 0;
            root.traverse((node) => {
