...
const JWT_ASSERTION_LIFETIME_SECONDS = 3600;
...
function createJwtAssertion(config: GoogleCalendarConfig): string {
    ...
    const expiresAt = issuedAt + JWT_ASSERTION_LIFETIME_SECONDS;
...
async function fetchAccessToken(
    ...
    const expiresAt =
        Date.now() +
        Math.max(result.expires_in - TOKEN_EXPIRY_BUFFER_SECONDS, 0) * 1000;
