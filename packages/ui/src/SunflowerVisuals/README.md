...
| `majstor_vrtlar` | Purple wheelbarrow |
...
`SunflowerMascot3D` is a rendered 3D-style **proposal**, shown only in Storybook. The existing sunflower SVG remains the default mascot and currency mark. This is transparent raster artwork, not a rigged or interactive 3D model.
...
- Mascot proposal: 768 × 768 WebP.
- Related birthday/refund event icons: 384 × 384 WebP under `../GameIcons/assets`.

The palette uses teal, green, blue, terracotta and purple; yellow belongs to sunflower petals rather than gold reward objects.
...
- `packages/ui/Icons/SunflowerVisuals`: each package, unknown-code fallback and current/proposed mascot comparison.
- `packages/game/icons/InGameIcons`: searchable artwork inventory.

Game earn/spend history reuses basket, calendar, gift, receipt, community, tasks, blocks and ruler artwork, plus new mint refund-arrow and birthday-cake icons. Existing achievement awards, block previews and the sunflower currency mark retain their identity.
