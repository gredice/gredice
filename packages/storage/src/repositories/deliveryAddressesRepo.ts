...
export function validateDeliveryAddress(
    data: Partial<InsertDeliveryAddress>,
): string[] {
    const errors: string[] = [];
    ...
    if (
        data.postalCode &&
        (data.postalCode.length < 3 || data.postalCode.length > 10)
    ) {
    ...
    if (data.countryCode && !/^[A-Z]{2}$/.test(data.countryCode)) {
        errors.push(
            'Country code must be ISO 3166-1 alpha-2 format (e.g., HR, US)',
        );
    }
