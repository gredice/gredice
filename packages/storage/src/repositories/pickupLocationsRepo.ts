...
import {
    type InsertPickupLocation,
    pickupLocations,
    type SelectPickupLocation,
    type UpdatePickupLocation,
} from '../schema';
...
export function getAllPickupLocations(): Promise<SelectPickupLocation[]> {
    return storage().query.pickupLocations.findMany({
        orderBy: [
            desc(pickupLocations.isActive),
            desc(pickupLocations.createdAt),
        ],
    });
}
...
export async function updatePickupLocation(
    ...
    if (!result[0]?.id) {
        throw new Error(
            'Failed to update pickup location - location not found',
        );
    }
}

// Deactivate a pickup location (soft delete)
export async function deactivatePickupLocation(
    locationId: number,
): Promise<void> {
    const result = await storage()
        .update(pickupLocations)
    ...
    if (!result[0]?.id) {
        throw new Error(
            'Failed to deactivate pickup location - location not found',
        );
    }
}

// Activate a pickup location
export async function activatePickupLocation(
    locationId: number,
): Promise<void> {
    const result = await storage()
        .update(pickupLocations)
    ...
    if (!result[0]?.id) {
        throw new Error(
            'Failed to activate pickup location - location not found',
        );
    }
}
