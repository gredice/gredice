...
export async function getAnalyticsTotals(days: number = 7) {
    ...
    const [
        ...
        storage().select({ count: count() }).from(accounts).where(lt(accounts.createdAt, beforeDate)),
        storage().select({ count: count() }).from(farms),
        storage().select({ count: count() }).from(farms).where(lt(farms.createdAt, beforeDate)),
        storage().select({ count: count() }).from(gardens),
        storage().select({ count: count() }).from(gardens).where(lt(gardens.createdAt, beforeDate)),
        storage().select({ count: count() }).from(gardenBlocks),
        storage().select({ count: count() }).from(gardenBlocks).where(lt(gardenBlocks.createdAt, beforeDate)),
        storage().select({ count: count() }).from(events),
        storage().select({ count: count() }).from(events).where(lt(events.createdAt, beforeDate)),
