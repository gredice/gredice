...
import { createTestAccount, createTestBlock, createTestGarden, createTestRaisedBed, ensureFarmId } from './helpers/testHelpers';
...
test('markCartPaidIfAllItemsPaid only marks paid if all items are paid', async () => {
    ...
    console.debug('Found cart after marking one item as paid:', JSON.stringify(foundCart));
